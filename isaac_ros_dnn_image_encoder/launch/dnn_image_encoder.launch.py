...
def launch_setup(context, *args, **kwargs):
    ...
    if not enable_padding:
        if network_image_width == 0 or network_image_height == 0:
            raise ValueError(
                'network_image_width and network_image_height must be > 0 '
...
def generate_launch_description():
    launch_args = [
        ...
        DeclareLaunchArgument(
            ...
            description='The interpolation method used for resizing',
        ),
        DeclareLaunchArgument(
            'encoding_desired',
            default_value='rgb8',
            description='The desired tensor image encoding',
        ),
        DeclareLaunchArgument(
            'image_input_topic',
            default_value='image',
        ...
        DeclareLaunchArgument(
            'tensor_output_topic',
            default_value='encoded_tensor',
            description='Output tensor topic name',
        ),
        DeclareLaunchArgument(
            'attach_to_shared_component_container',
            default_value='False',
            description='Whether to load components into an existing container',
        ),
        DeclareLaunchArgument(
            'component_container_name',
            default_value='dnn_image_encoder_container',
            description='The target component container name',
        ),
        DeclareLaunchArgument(
            'tensor_name',
        ...
        DeclareLaunchArgument(
            'final_tensor_name',
            default_value=LaunchConfiguration('tensor_name'),
