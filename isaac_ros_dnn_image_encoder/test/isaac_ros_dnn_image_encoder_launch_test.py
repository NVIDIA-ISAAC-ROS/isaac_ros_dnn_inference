...
def generate_test_description():
    """Generate launch description that runs dnn_image_encoder.launch.py."""
    namespace = IsaacROSDnnImageEncoderLaunchTest.generate_namespace()
    ...
    return IsaacROSDnnImageEncoderLaunchTest.generate_test_description([
        dnn_image_encoder_launch,
    ])


class IsaacROSDnnImageEncoderLaunchTest(IsaacROSBaseTest):
    """Test the DNN image encoder launched by dnn_image_encoder.launch.py."""

    filepath = pathlib.Path(os.path.dirname(__file__))

    def test_launch_maintain_aspect_ratio(self):
        """
        Test dnn_image_encoder.launch.py: crop to 1080x1080, resize to 512x512, NCHW output.

        Publishes image and camera_info, subscribes to tensors, and verifies
        the output tensor has shape [1, 3, 512, 512] and matching timestamp.
        """
        TIMEOUT = 300
        received_messages = {}
